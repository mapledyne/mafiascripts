...
notify bumcheekcity;
since r16052; // Version where the maximizer string 'effective' was added
...
boolean bcascGuild() {
	if ((!in_hardcore() && !bcasc_RunSCasHC) || my_path() == "Avatar of Boris" || my_path() == "Zombie Slayer" || my_path() == "Avatar of Jarlsberg" || my_path() == "Avatar of Sneaky Pete" || my_path() == "Actually Ed the Undying") return checkStage("guild", true);
...
boolean bcascMacguffinHiddenCity() {
	...
	if (item_amount($item[stone triangle]) == 0) {
		...
		while (item_amount($item[dripping stone sphere]) == 0) {
			...
			if (available_amount($item[surgical apron]) > 0 && !contains_text(to_string(equipped_item($slot[shirt])), "Sneaky Pete")) {
...
//				}
			...
			case "beach" :
				...
				while (i_a("barrel of gunpowder") < 5) {
					...
					bumAdv($location[Sonofa Beach], "", "items", "5 barrel of gunpowder", "Getting the Barrels of Gunpowder", "+");
				}
				cli_execute("outfit "+bcasc_warOutfit);
