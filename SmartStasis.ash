...
void build_custom() {
  ...
  // flyers
   foreach flyer in $items[jam band flyers, rock band flyers] if (item_amount(flyer) > 0 && get_property("questL12War") != "finished" && be_good(flyer) && get_property("flyeredML").to_int() < 10050 &&
...
*/
  ...
  // under the sea
   switch (where.zone) {
      ...
      case "The Glaciest": if (get_property("walfordBucketItem") == "blood" && have_equipped($item[walford's bucket]) && $strings[started, step1] contains get_property("questECoBucket") &&
...
void enqueue_custom() {
   ...
   foreach n,ev in custom {
      ...
      boolean stunfirst = (adj.stun + ev.stun < 1 && !ev.endscombat && to_profit(stun_action(contains_text(ev.id,"use "))) >= 0 && buytime.id != "");  // should we stun?
...
string stasis() {
   if ($monsters[naughty sorority nurse, naughty sorceress, naughty sorceress (2),
       pufferfish, bonerdagon, toxic beastie] contains m) return page;    // never stasis these monsters
