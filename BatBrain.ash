...
void build_blacklist() {
   ...
   if (item_amount($item[fizzing spore pod]) > 0 && $strings[started,step1,step2,step3,step4,step5,step6,step7,step8,step9,step10,step11,step12,step13] 
      contains get_property("questG04Nemesis")) temp_blacklist_inc("use 8427",6);
...
if (get_property("_meatperhp") != "") meatperhp = to_float(get_property("_meatperhp"));
                          ...
                          get_property("questM24Doc") == "finished" ? 6 : 9);   // best of galaktik, scroll of drastic (hey, they rhyme)
...
if (get_property("_meatpermp") != "") meatpermp = to_float(get_property("_meatpermp"));
   ...
   if (my_primestat() == $stat[mysticality] || (my_class() == $class[accordion thief] && my_level() > 8))
    ...
    else meatpermp = get_property("questM24Doc") == "finished" ? 6 : (dispensary_available() ? 8 : 9);
 }
...
spread regular(int ts) {  // 1) norm, 2) thrust-smack / axing, 3) lts, 5) bashing slam smash / cleave
   ...
   res[$element[none]] = max(0,max(0,floor(my_stat(to_string(current_hit_stat()))*ltsadj*radj) - monster_stat("def")) +
      (unarmed() ? 1 : max(1,get_power(equipped_item($slot[weapon]))*0.15 + 0.5) * 
...
advevent stasis_action() {  // returns most profitable lowest-damage action
   ...
   foreach i,opt in opts {  // skip multistuns, custom, and non-multi-usable items
      ...
      vprint("Stasis action chosen: "+opt.id+" (profit: "+rnum(opt.profit)+")",8);
      plink = opt;
      return opt;
   }
   plink = new advevent;
...
//      map_to_file(mdb,"monstercache.txt");     // uncomment for debugging
  ...
  // special monster attributes
   ...
   void setmatt(string attr, string val) {
      switch (attr) {
         case "seal": mdb[m].onlyhurtby = "club"; vprint("This monster is a seal, and can only be damaged by clubs.",8); break; // <- delete after converting "seal"
...
//            case "damincreaser": setmatt("",""); break;    // increased attack damage
            ...
            case "musdebuffer": case "magdebuffer": case "moxdebuffer": break;
...
string act(string action) {
  ...
  // set combat round and record happenings
   if (my_path() != "One Crazy Random Summer" && !equals(to_string(m), get_property("lastEncounter")) && 
